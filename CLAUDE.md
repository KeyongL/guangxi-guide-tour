...
- 2026-07-14 | 排查"地图不显示"改完 RouteMap.astro 的 `<script>` 块后，用同一个未重启的 `npm run dev` 进程 curl 验证——终端已打出 `[watch]` 检测到变更，但吐出的编译产物仍是改动前的旧版本 | 长期不重启的 dev 进程，改 `.astro` 组件 `<script>` 块后有概率吃 Vite 陈旧编译缓存（哪怕 watcher 日志已确认收到事件）。验证代码改动前先 kill 旧进程、重开一个全新 dev server，不要复用；这也是过去几次"代码看着没问题、页面表现却对不上"类报告的合理成因之一。
- 2026-07-08 | 首次上线 Cloudflare 踩了一串坑，逐条记下——下次直接照做 | **部署 playbook（站长本机在输入框里逐行跑，`!` 开头即本机执行）**：① 本环境无 `gh` CLI，别用 `gh auth login`；GitHub 认证走 **SSH deploy key**（`ssh-keygen -t ed25519` 生成→公钥贴到仓库 Settings→Deploy keys→**勾 Allow write access**）。② **铁律#7：AI 不能 push**，settings 会拦截含 push 的 Bash——推送命令必须由站长在输入框 `! git push ...` 亲手跑。③ 本地分支名带 worktree 前缀，`git push` 会报 upstream 不同名 → 用 `! git push origin HEAD:main`。④ GitHub 建仓库若勾了 Add README，远端有初始提交致 push rejected → 首推用 `! git push origin HEAD:main --force`（远端仅自动 README，覆盖零损失；仅首推可 force）。⑤ **Cloudflare 若建成 Worker 而非 Pages**（部署命令是 `npx wrangler deploy`），需仓库根加 `wrangler.jsonc`：`{name:"<与Worker同名>", compatibility_date, assets:{directory:"./dist"}}`，否则报 "Worker already exists, could not confirm update"。⑥ 提交前 `npx wrangler deploy --dry-run` 本地验证配置。⑦ 之后每次改内容 = 站长 `! git push origin HEAD:main`，Cloudflare 自动重建，无需再配。⚠️ 令牌/密钥别在对话明文发；若发了，用完立即去 GitHub 撤销。
